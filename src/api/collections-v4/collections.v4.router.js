...
const collectionsV4Router = () => {
  ...
  router.get("/unified", async (req, res, next) => {
    logEmitter.emit("functionCall", "registrations.v4.router", "GET /unified route");
    try {
      let registrations;
      const options = {
      ...
      logEmitter.emit("functionSuccess", "registrations.v4.router", "GET /unified route");
      res.send(registrations);
    } catch (err) {
      logEmitter.emit("functionFail", "registrations.v4.router", "GET /unified route", err);
      next(err);
    }
  });

  router.get("/:subscriber", async (req, res, next) => {
    logEmitter.emit("functionCall", "registrations.v4.router", "/:subscriber route");
    try {
      ...
      logEmitter.emit("functionSuccess", "registrations.v4.router", "GET /:subscriber route");
      res.send(registrations);
    } catch (err) {
      logEmitter.emit("functionFail", "registrations.v4.router", "GET /:subscriber route", err);
      next(err);
    }
  });

  router.get("/:subscriber/:fsa_rn", async (req, res, next) => {
    logEmitter.emit("functionCall", "registrations.v4.router", "GET /:subscriber/:fsa_rn route");
    try {
      const options = {
  ...
  router.put("/:subscriber/:fsa_rn", async (req, res, next) => {
    logEmitter.emit("functionCall", "registrations.v4.router", "PUT /:subscriber/:fsa_rn route");
    try {
      const options = {
