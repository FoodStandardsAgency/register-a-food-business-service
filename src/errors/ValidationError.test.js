...
describe("Constructor: ValidationError", () => {
  ...
  it("creates error array on state when none exists", () => {
    ...
    expect(response.state.establishment_postcode[0]).toBe("Invalid establishment postcode");
  });
  ...
  it("creates multiple errors on the same array", () => {
    ...
    expect(response.state.establishment_postcode[0]).toBe("Invalid establishment postcode");
    expect(response.state.establishment_postcode[1]).toBe("Postcode must not contain symbols");
