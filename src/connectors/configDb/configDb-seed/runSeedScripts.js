...
const runSeed = async () => {
  ...
  const collectionToSeed = await inquirer.prompt(questions.general.collectionQuestion);

  // get the environments that the user wants to seed to
  const environmentsToSeed = await inquirer.prompt(questions.general.envQuestions);

  // get the configDB urls for these environments, and confirm that they are correct
  const requiredEnvUrlQuestions = questions.general.envUrlQuestions.filter((envQ) =>
    environmentsToSeed.environments.includes(envQ.env)
  );
  let environmentUrls;
  ...
  while (environmentUrlsAreCorrect.confirmation_env_urls === false) {
    ...
    environmentUrlsAreCorrect = await inquirer.prompt(questions.general.confirmEnvUrls);
  }
  ...
  const requiredCollectionQuestions = allCollectionQuestions.filter((collectionQ) =>
    environmentsToSeed.environments.some((envName) => collectionQ.env.includes(envName))
  );
  let seedData;
  ...
  while (seedDataIsCorrect.confirmation_seed_data === false) {
    ...
    seedDataIsCorrect = await inquirer.prompt(questions.general.confirmSeedData);
  }
  ...
  for (let index in environmentsToSeed.environments) {
    try {
      ...
      const specifiedCollection = configDB.collection(collectionToSeed.collectionName);
      ...
      const populatedTemplate = templates[collectionToSeed.collectionName](seedData, envName);
