...
const copyData = async (queryInterface, transaction) => {
  return Promise.all([
    queryInterface.sequelize.query(
      "UPDATE registrations.operators " +
...
const recopyData = async (queryInterface, transaction) => {
  return Promise.all([
    queryInterface.sequelize.query(
      "UPDATE registrations.operators SET operator_first_line = operator_address_line_1, " +
        "operator_street = operator_address_line_2, " +
        "operator_dependent_locality = operator_address_line_3",
      { transaction: transaction }
    ),
    queryInterface.sequelize.query(
      "UPDATE registrations.premises SET establishment_first_line = establishment_address_line_1, " +
