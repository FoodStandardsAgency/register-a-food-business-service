...
const applyCosmosBusinessTypeTransforms = async (registration, transform, newStatus) => {
  logEmitter.emit(
    "info",
  ...
  try {
    ...
    logEmitter.emit("info", `Failed to update BE Cache FSA-RN: ${registration["fsa-rn"]} ${err.message}`);
    await beCache.updateOne(
      { "fsa-rn": registration["fsa-rn"] },
      {
        $set: {
  ...
  await queryInterface.createTable(
    ...
    await queryInterface.sequelize.query(
      ...
      const promises = regs.map(async (reg) => {
        try {
          await applyPgBusinessTypeTransforms(reg, transformToKey).then(async () => {
            await queryInterface.sequelize.query(`INSERT INTO registrations."tmp281MigrationStatus" ("registrationId", "status") values (${reg.id}, 'true')`).then(() => {
              logEmitter.emit("info", `Successfully updated Temp-Store ID: ${reg.id}`);
            });
          });
        } catch (err) {
          logEmitter.emit("info", `Failed to update Temp-Store ID: ${reg.id} ${err.message}`);
          await queryInterface.sequelize.query(`INSERT INTO registrations."tmp281MigrationStatus" ("registrationId", "status") values (${reg.id}, 'Error during enum migration: ${err.message}')`);
        }
      });
...
const migratePgBusinessTypesFromEnums = async (queryInterface) => {
  ...
  await queryInterface.sequelize.query(
    ...
    const promises = regs.map(async (reg) => {
      try {
        await applyPgBusinessTypeTransforms(reg, transformToValue).then(async () => {
          await queryInterface.sequelize.query(`DELETE FROM registrations."tmp281MigrationStatus" WHERE "registrationId"=${reg.id})`).then(() => {
            logEmitter.emit("info", `Successfully updated Temp-Store ID: ${reg.id}`);
          });
        });
      } catch (err) {
        logEmitter.emit("info", `Failed to update Temp-Store ID: ${reg.id} ${err.message}`);
        await queryInterface.sequelize.query(`INSERT INTO registrations."tmp281MigrationStatus" ("registrationId", "status") values (${reg.id}, 'Error during enum un-migration: ${err.message}')`);
      }
    });
    await Promise.allSettled(promises);
  });
...
const migrateCosmosBusinessTypesToEnums = async () => {
  ...
  const registrations = await beCache.find({"migration-2-8-1-enums-status":{ $ne: true }, "establishment.activities.business_type":{ $in: ['Commercial bakery', 'Hostel or bed and breakfast ', 'Childcarer, nursery or play group']}}).toArray();
...
const migrateCosmosBusinessTypesFromEnums = async () => {
  ...
  const registrations = await beCache.find({"migration-2-8-1-enums-status": true}).toArray();
...
module.exports = { migratePgBusinessTypesToEnums, migratePgBusinessTypesFromEnums, migrateCosmosBusinessTypesToEnums, migrateCosmosBusinessTypesFromEnums };
